...
exports.TestSlotService = void 0;
const common_1 = require("@nestjs/common");
const client_1 = require("@prisma/client");
const postmark = require("postmark");
const prisma_service_1 = require("../prisma.service");
let TestSlotService = class TestSlotService {
    constructor(prisma) {
        this.prisma = prisma;
    ...
    async create(testSlotDto) {
        return await this.prisma.$transaction(testSlotDto.map((slot) => this.prisma.testSlot.create({
            data: {
    ...
    async delete(id) {
        ...
        if (internsOnSlotCount)
            throw new common_1.BadRequestException('Test with linked interns cannot be removed! Try manually cancelling intern test slots.');
    ...
    async getAvailableSlots(internId, discipline) {
        ...
        if (!internDiscipline) {
            throw new common_1.NotFoundException('Intern with such discipline not found');
        }
        if (internDiscipline.testSlot) {
            throw new common_1.BadRequestException('Test already scheduled');
        }
        if (internDiscipline.testStatus !== client_1.TestStatus.PickTerm) {
            throw new common_1.BadRequestException('Intern has no right to schedule test');
        }
        ...
        const availableSlots = slots
            ...
            .map((s) => (Object.assign(Object.assign({}, s), { password: undefined })));
    ...
    async scheduleTest(slotId, internId) {
        ...
        if (internDiscipline.testStatus !== client_1.TestStatus.PickTerm) {
            throw new common_1.NotFoundException('Intern has no right to pick this slot');
        }
...
DUMP Udruga mladih programera`,
        ...
        return await this.prisma.internDiscipline.update({
            ...
            data: {
                testStatus: client_1.TestStatus.Pending,
                testSlot: {
                    connect: {
                        id: slotId,
                    },
    ...
    async chooseTest(password) {
        ...
        if (!testSlot)
            throw new common_1.BadRequestException('Such test does not exist!');
        return testSlot;
    }
    async startTest(testSlotId, email, password) {
        ...
        if (!internDiscipline) {
            throw new common_1.BadRequestException('Test does not exist or intern does not have permission to access');
        }
        ...
        if (new Date() < slot.start) {
            throw new common_1.BadRequestException('Test not started yet');
        }
        return slot;
    }
    async submitTest(testSlotId, test) {
        const internDiscipline = await this.prisma.internDiscipline.findFirst({
            where: {
                intern: {
                    email: {
        ...
        if (!internDiscipline) {
            throw new common_1.BadRequestException('Test does not exist or intern does not have permission to submit');
        }
        ...
        await this.prisma.internDiscipline.update({
            ...
            data: {
                testStatus: client_1.TestStatus.Done,
    ...
    async getTestAnswersByIntern(testSlotId, internId) {
        ...
        if (!internDiscipline) {
            throw new common_1.BadRequestException('Test does not exist or is not done');
        }
...
exports.TestSlotService = TestSlotService;
exports.TestSlotService = TestSlotService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], TestSlotService);
//# sourceMappingURL=test-slot.service.js.map